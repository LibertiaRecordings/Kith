...
export default function ServicesPage() {
  ...
  return (
    <main id="main" className="container mx-auto px-6 py-10 min-h-screen bg-ink text-paper">
      <h1 className="text-4xl md:text-5xl font-display font-semibold tracking-tight text-paper">Our Services</h1>
      <p className="mt-3 text-chrome font-mono">Crafted for precision, designed for you.</p>

      <section className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service) => (
          <div key={service.slug} className="bg-graphite rounded-2xl p-6 shadow-ultra-soft">
            <h2 className="text-2xl font-display font-medium text-paper">{service.name}</h2>
            <p className="text-neon font-mono text-lg mt-2">${service.price} <span className="text-chrome text-sm">({service.duration} min)</span></p>
            <p className="text-chrome mt-4 leading-relaxed">{service.description}</p>

            {service.addOns.length > 0 && (
              <div className="mt-4 pt-4 border-t border-chrome/20">
                <h3 className="text-lg font-display font-medium text-paper mb-2">Add-ons:</h3>
                <ul className="space-y-2">
                  {service.addOns.map((addon, index) => (
                    <li key={index} className="flex justify-between items-center text-chrome text-sm">
