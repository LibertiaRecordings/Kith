...
export default function BarbersPage() {
  ...
  const barbers = [
    ...
    { slug: "liam", name: "Liam", role: "Barber", specialties: ["Fades", "Tapers", "Textured Hair"], imageUrl: "/images/liam.jpeg" }, // Corrected Liam's image path
      ...
      <section className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {barbers.map((barber) => (
          <Link href={`/barbers/${barber.slug}`} key={barber.slug} className="block">
            <div className="bg-card rounded-2xl p-6 shadow-ultra-soft can-animate group flex flex-col h-[26rem]"> {/* Ensured fixed height */}
              ...
              <ul className="mt-4 flex flex-wrap justify-center gap-2 h-[4.5rem] overflow-hidden">
                {barber.specialties.map((spec, index) => (
                  <li key={index} className="bg-background text-ink text-sm px-3 py-1 rounded-full border border-muted-foreground/30 group-hover:border-primary transition-colors font-body"> {/* Corrected text color to text-ink for visibility */}
                    {spec}
                  </li>
                ))}
              </ul>
