...
export default function BarbersPage() {
  ...
  return (
    <main id="main" className="container mx-auto px-6 py-16 min-h-screen bg-background text-foreground">
      <h1 className="text-4xl md:text-5xl font-display font-normal tracking-tight text-foreground">Our Barbers</h1>
      <p className="mt-3 text-muted-foreground font-display text-lg">Meet the hands behind the precision.</p>
        ...
        {barbers.map((barber) => (
          <Link href={`/barbers/${barber.slug}`} key={barber.slug} className="block">
            <div className="bg-card rounded-2xl p-6 shadow-ultra-soft can-animate group">
              ...
              <p className="text-muted-foreground text-center font-display text-base mt-1">{barber.role}</p>
              <ul className="mt-4 flex flex-wrap justify-center gap-2">
                {barber.specialties.map((spec, index) => (
                  <li key={index} className="bg-background text-muted-foreground text-sm px-3 py-1 rounded-full border border-muted-foreground/30 group-hover:border-primary transition-colors">
                    {spec}
                  </li>
                ))}
              </ul>
              <div className="mt-4 text-center">
                <span className="inline-flex items-center justify-center px-4 py-2 border border-muted-foreground/30 text-muted-foreground rounded-full text-sm font-medium group-hover:border-primary group-hover:text-primary transition-colors">
                  View Profile
                </span>
              </div>
            </div>
