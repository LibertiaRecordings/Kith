...
const uploadMixFormSchema = z.object({
  ...
  audioFile: z
    .any() // Start with z.any() to allow undefined initially
    .refine((file) => file instanceof File, { message: 'Audio file is required.' }) // Refine to ensure it's a File
...
const AdminUploadMixForm: React.FC = () => {
  ...
  const form = useForm<UploadMixFormValues>({
    ...
    defaultValues: {
      title: '',
      artist: '',
      audioFile: undefined, // Explicitly undefined to match z.any() initial state
  ...
  const onSubmit = async (values: UploadMixFormValues) => {
    ...
    try {
      const result = await uploadRadioTrack({
        ...
        file: values.audioFile, // No need to cast, as schema ensures it's a File at this point
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Enter artist name" className="bg-background border-muted-foreground/30 text-foreground focus:ring-primary" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
